...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Menus, ComCtrls, Gauges, ImgList;
...
i:integer;
begin
for i:=0 to High(Coords) do FindPanelByPos(33*Coords[i].X+144,33*Coords[i].Y+40).Color:=clsilver;
if SelPan<>nil then begin
 SelPan.Caption:=' ';
 SelPan.Color:=clsilver;
 SelPan:=nil;
 SetLength(Coords,0);
end;
end;
...
procedure TForm1.SaveGame(Filename:string);
var
str:string;
i,x,y:integer;
begin
 ...
 Rewrite(output);
 Writeln(Desk[0,2]+Desk[1,2]+Desk[2,2]+Desk[3,2]+Desk[4,2]);
 Writeln(ListBox1.count);
 for i:=0 to ListBox1.count-1 do begin
  Str:=Trim(Copy(Listbox1.items[i],1,Length(Listbox1.items[i])-Pos(' ',Listbox1.items[i])-1));
  Writeln(Str);
 end;
 Writeln(ListBox2.count);
 for i:=0 to ListBox2.count-1 do begin
  Str:=Trim(Copy(Listbox2.items[i],1,Length(Listbox2.items[i])-Pos(' ',Listbox2.items[i])-1));
  Writeln(Str);
 end;
...
for i:=0 to High(MP[index]) do begin  //Поиск панели с координатами 33*MP[index][i].X+118,33*MP[index][i].Y+30 и выделяем ее цветом
 P:=FindPanelByPos(33*MP[index][i].X+144,33*MP[index][i].Y+40);
 P.Color:=clred;
...
begin
 ...
 1: begin
  ...
  //Gauge1.Progress:=Round((100*S1)/(S1+S2));
end;
...
{if Index<=Listbox2.Count-1 then begin
 if Listbox1.Items[index]=Listbox2.Items[index] then Font.Color:=clyellow else
 if Listbox1.Items[index]>Listbox2.Items[index] then Font.Color:=cllime else
 if Listbox1.Items[index]<Listbox2.Items[index] then Font.Color:=clmaroon;
end else Font.Color:=clyellow; }
Font.Color:=clyellow;
FillRect(Rect);
...
maxlen:=(Sender as TMenuItem).Tag;
end;

procedure TForm1.N16Click(Sender: TObject);
begin
Showmessage('Автор программы: Бышин Артем'+#13#10+
            'Мой ящик: ArtPost-X@mail.ru'+#13#10+
